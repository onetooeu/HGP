...
// HGPeDU hosted mode: gateway lives on portal.onetoo.eu
const AMS_GATEWAY_ORIGIN = "https://portal.onetoo.eu";
...
function resolveGatewayUrl(path){
  if (!path) return null;
  try{
    return new URL(path, AMS_GATEWAY_ORIGIN).toString();
  }catch{
    return String(path);
  }
}
...
async function loadGatewaySpec(){
  ...
  setGwState("loading…");
  try{
    const r = await fetch(AMS_GATEWAY_ORIGIN + "/.well-known/ams-gateway-spec.json", { cache:"no-store" });
